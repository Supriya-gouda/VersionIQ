...
function Settings() {
  ...
  useEffect(() => {
    if (typeof document !== "undefined") {
      ...
      const nextTheme = savedTheme === "dark" ? "dark" : document.documentElement.classList.contains("dark") ? "dark" : "light";
      setTheme(nextTheme);
      document.documentElement.classList.toggle("dark", nextTheme === "dark");
    }

    getQuota().then(res => setQuota(res.quota)).catch(console.error);
  }, []);
      ...
      <div className="grid lg:grid-cols-3 gap-6 mt-6">
        ...
        <div className="lg:col-span-2 rounded-xl border border-border bg-card shadow-card p-6">
          ...
          <div className="mt-5 flex items-center gap-5">
              <div className="relative">
                <div className="w-20 h-20 rounded-full bg-gradient-primary grid place-items-center text-primary-foreground text-2xl font-semibold shadow-elegant overflow-hidden">
                  {user.avatar ? <img src={user.avatar} className="w-full h-full object-cover" alt="Avatar" /> : user.name.slice(0, 2).toUpperCase()}
                </div>
                <button 
                  type="button" 
          ...
          <form onSubmit={handleSaveProfile} className="grid sm:grid-cols-2 gap-4 mt-6">
            ...
            <Field label="Email" value={user.email} onChange={() => {}} className="sm:col-span-2" disabled />
            <div className="sm:col-span-2 flex justify-end">
              <button disabled={saving} className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-gradient-primary text-primary-foreground text-sm font-medium shadow-elegant disabled:opacity-50">
        ...
        <div className="rounded-xl border border-border bg-card shadow-card p-6">
          ...
          <div className="mt-5">
            {quota ? (
              <>
                <div className="text-3xl font-semibold">{(quota.used / (1024 ** 3)).toFixed(2)} <span className="text-base text-muted-foreground font-normal">/ {(quota.limit / (1024 ** 3)).toFixed(0)} GB</span></div>
                <progress className="w-full h-2 mt-3" value={quota.percent} max={100} />
                <div className="mt-2 text-xs text-muted-foreground text-right">{quota.percent}% used</div>
              </>
            ) : (
            ...
            <ul className="mt-5 space-y-3 text-sm">
              {[
                { label: "Live Data", val: quota ? `${(quota.used / (1024 ** 2)).toFixed(1)} MB` : "...", color: "bg-primary" },
          ...
          <ul className="mt-5 divide-y divide-border">
            {([
              ...
              <li key={key} className="flex items-center justify-between py-4">
                ...
                <Toggle label={title} on={notifs[key]} onChange={(v) => setNotifs({ ...notifs, [key]: v })} />
              </li>
            ))}
          </ul>
        </div>
        ...
        <div className="rounded-xl border border-border bg-card shadow-card p-6">
          ...
          <p className="mt-1 text-sm text-muted-foreground">Sign out of this browser to clear the stored token.</p>
          <button onClick={logout} className="mt-5 inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-border hover:bg-muted text-sm font-medium">
...
function Field({ label, value, onChange, className = "", disabled = false }: { label: string; value: string; onChange: (v: string) => void; className?: string; disabled?: boolean }) {
  return (
