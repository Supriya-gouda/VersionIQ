...
import { getSummary, listFiles, listVersions, compareVersions, type ApiFile, type ApiVersion } from "@/lib/api";
      ...
      <div className="grid lg:grid-cols-3 gap-6 mt-6">
              ...
              <li key={file._id}>
                ...
                >
                  ...
                  <div className="min-w-0 text-left">
                    ...
                    <div className="text-xs text-muted-foreground">{file.currentVersionNumber} versions</div>
                  </div>
                </button>
              </li>
            ))}
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                  ...
                  {summary.source && (
                    <Badge variant={summary.source === "local" ? "outline" : "secondary"} className="text-[10px] uppercase h-5">
                      Powered by {summary.source} {summary.model && `(${summary.model})`}
                    </Badge>
                  )}
                </div>
                <div className="font-semibold text-lg">{activeFile?.originalName ?? "Select a file"} - v{activeVersion?.versionNumber ?? 0}</div>
                ...
                <p className="mt-4 text-base leading-relaxed font-medium">{summary.text || activeVersion?.summary || "No summary available"}</p>
                
                {summary.aiDetails?.extraNotes && (
                   <p className="mt-2 text-sm text-muted-foreground border-l-2 border-border pl-3">{summary.aiDetails.extraNotes}</p>
                )}

                <div className="mt-6 flex flex-wrap gap-3 text-sm">
                  <Pill icon={Plus} color="text-success bg-success/10">{activeVersion?.diffStats.added ?? 0} additions</Pill>
                  <Pill icon={Minus} color="text-destructive bg-destructive/10">{activeVersion?.diffStats.removed ?? 0} removals</Pill>
                  <Pill icon={Pencil} color="text-warning-foreground bg-warning/15">{activeVersion?.diffStats.modified ?? 0} modifications</Pill>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            <DetailBox tone="success" title="Significant Additions" items={summary.aiDetails?.addedLines || []} />
            <DetailBox tone="destructive" title="Significant Removals" items={summary.aiDetails?.removedLines || []} />
            <DetailBox tone="warning" title="Logical Modifications" items={summary.aiDetails?.modifiedLines || []} />
          </div>
              ...
              <div className="flex items-center gap-3">
                {diffResult && (
                  <div className="text-[10px] text-muted-foreground uppercase font-mono hidden sm:block">
                    v{diffResult.v1.number} <span className="mx-1">{"->"}</span> v{diffResult.v2.number}
                  </div>
                )}
                ...
                <pre className="rounded-lg bg-muted/50 border border-border p-4 text-xs font-mono overflow-x-auto leading-6 max-h-[400px]">
                  {diffResult?.textDiff ? (
                    diffResult.textDiff.split("\n").map((line: string, i: number) => (
                      <div key={i} className={line.startsWith("+") ? "text-success" : line.startsWith("-") ? "text-destructive" : ""}>
                        {line}
                      </div>
                    ))
                  ) : (
                    <div className="text-muted-foreground italic">Initial version - no previous version to compare against.</div>
                  )}
                </pre>
              </div>
            )}
