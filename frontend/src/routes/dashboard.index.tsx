...
import { ApiFile, ApiPipelineLog, ApiActivity, listFiles, listPipelines, listActivities } from "@/lib/api";
...
function Dashboard() {
  ...
  const stats = useMemo(() => {
    ...
    return [
      { label: "Total files", value: String(files.length), delta: "Live", icon: Files, tone: "text-primary bg-primary/10" },
      { label: "Versions", value: versionCount.toLocaleString(), delta: "Live", icon: GitBranch, tone: "text-info bg-info/10" },
      { label: "Stable files", value: String(stableCount), delta: "Live", icon: ShieldCheck, tone: "text-success bg-success/10" },
      { label: "Latest pipeline", value: latestPipelineStatus.toUpperCase(), delta: "Live", icon: Hammer, tone: "text-warning bg-warning/15" },
    ];
  }, [files, pipelines]);
    ...
    <DashboardLayout>
      <div className="flex items-end justify-between flex-wrap gap-4 mb-8">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Live file, version, and pipeline activity across your vault.</p>
        </div>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {loading ? Array.from({ length: 4 }).map((_, index) => (
          <div key={index} className="rounded-xl border border-border bg-card p-5 shadow-card animate-pulse">
      ...
      <div className="grid lg:grid-cols-3 gap-6 mt-6">
        ...
        <div className="lg:col-span-2 rounded-xl border border-border bg-card shadow-card">
          ...
          {loading ? (
            ...
            <div className="p-10 text-center text-sm text-muted-foreground">No files uploaded yet.</div>
          ) : (
            <ul className="divide-y divide-border">
              {recentFiles.map((file) => (
                <li key={file._id} className="p-4 flex items-center gap-4 hover:bg-muted/50 transition-smooth">
                  <FileIcon type="yaml" />
                  <div className="flex-1 min-w-0">
                    <div className="font-medium truncate">{file.originalName}</div>
                    <div className="text-xs text-muted-foreground">
                      {(file.size / 1024).toFixed(1)} KB · updated {new Date(file.updatedAt).toLocaleDateString()} · {file.currentVersionNumber} versions
                    </div>
                  </div>
                  <Badge variant={file.currentVersionId?.status ?? "neutral"}>{file.currentVersionId?.status ?? "neutral"}</Badge>
                </li>
              ))}
            </ul>
          )}
          ...
          {loading ? (
            ...
            <ul className="p-2">
              {recentActivity.map((a) => {
                ...
                return (
                  <li key={a._id} className="flex gap-3 p-3 rounded-lg hover:bg-muted/50 transition-smooth">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-muted-foreground truncate">{a.fileName} {a.details}</div>
      ...
      <div className="mt-6 rounded-xl border border-border bg-card shadow-card">
        ...
        {loading ? (
          ...
          <div className="p-10 text-center text-sm text-muted-foreground">No pipeline logs found.</div>
        ) : (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 p-4">
            {recentPipelineCards.map((pipeline) => (
              <div key={pipeline._id} className="rounded-lg border border-border p-4 hover:shadow-card transition-smooth">
                <div className="flex items-center justify-between">
                  <div className="font-medium text-sm">{pipeline.pipeline}</div>
                  <Badge variant={pipeline.status === "success" ? "success" : pipeline.status === "failed" ? "failed" : pipeline.status === "running" ? "running" : "neutral"}>{pipeline.status}</Badge>
                </div>
                <div className="text-xs text-muted-foreground mt-2 font-mono">#{pipeline.buildNumber} · {pipeline.author || "Jenkins"}</div>
                <div className="text-xs text-muted-foreground mt-1">{pipeline.durationMs ? `${Math.round(pipeline.durationMs / 1000)}s` : "Pending"} · {pipeline.startedAt ? new Date(pipeline.startedAt).toLocaleDateString() : "No date"}</div>
              </div>
            ))}
          </div>
        )}
