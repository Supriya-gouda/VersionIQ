...
const mongodbUri =
  ...
  "mongodb://admin:changeme@localhost:27018/version_vault?authSource=admin";
...
async function regenerate() {
  try {
    ...
    for (let i = 0; i < versions.length; i++) {
      ...
      console.log(
        `[${i + 1}/${versions.length}] Processing ${version.originalName} (Version ${version.versionNumber})...`,
      );
      ...
      if (version.versionNumber > 1) {
        ...
        if (previousVersion) {
          previousContent = await readFileContent(
            previousVersion.storagePath,
            previousVersion.mimeType,
            previousVersion.originalName,
          );
        }
      }
